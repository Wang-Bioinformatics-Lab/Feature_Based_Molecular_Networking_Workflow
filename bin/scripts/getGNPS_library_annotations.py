...
# NP Classifier
# Cached on the SMILES string so we only hit the API once per unique structure,
# regardless of how many library hits share it.
@lru_cache(maxsize=None)
def _get_npclassifier_classifications(smiles):
    try:
        npclassifier_url = "https://npclassifier.gnps2.org/classify?smiles={}".format(urllib.parse.quote_plus(smiles))
        r = requests.get(npclassifier_url, timeout=10)
        r.raise_for_status()
        classification_json = r.json()

        return ("|".join(classification_json["superclass_results"]),
                "|".join(classification_json["class_results"]),
                "|".join(classification_json["pathway_results"]))
    except KeyboardInterrupt:
        raise
    except:
        return ("N/A", "N/A", "N/A")
...
def _enrich_npclassifier(output_result_dict):
    smiles = str(output_result_dict.get("Smiles", ""))

    if len(smiles) > 5 and smiles != "N/A":
        npclassifier_superclass, npclassifier_class, npclassifier_pathway = _get_npclassifier_classifications(smiles)
    else:
        npclassifier_superclass, npclassifier_class, npclassifier_pathway = "N/A", "N/A", "N/A"

    output_result_dict["npclassifier_superclass"] = npclassifier_superclass
    output_result_dict["npclassifier_class"] = npclassifier_class
    output_result_dict["npclassifier_pathway"] = npclassifier_pathway

    return output_result_dict


# Here we will enrich the smiles
def _enrich_annotations(output_result_dict, npclassifier=True):
    # Calculating inchi
    ...
    if npclassifier:
        output_result_dict = _enrich_npclassifier(output_result_dict)
...
def enrich_output(input_filename, output_filename, 
    ...
    for result_obj in tqdm(input_results_df.to_dict(orient="records")):
        ...
        if forceoffline:
            ...
            # The offline path skips _enrich_annotations entirely, so run NP Classifier on its own
            if npclassifier:
                output_result_dict = _enrich_npclassifier(output_result_dict)
        else:
            ...
            try:
                output_result_dict = _enrich_annotations(output_result_dict, npclassifier=npclassifier)
            except:
                pass
...
def main():
    ...
    parser.add_argument("--npclassifier", default="Yes", type=str, help="Annotate library hits with NP Classifier pathway/superclass/class, Yes or No")
    ...
    enrich_output(input_result_filename, output_result_filename, topk=args.topk, 
                    ...
                    npclassifier=(args.npclassifier == "Yes"))
